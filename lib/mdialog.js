    ...
    show: function() {
        setTimeout(function() {
            ...
            if (!this._bgLock) {
                ...
                this._bodyWidth = GetStyle(document.body,
                    'width');
                ...
                this._bodyOverflow = GetStyle(document.body,
                    'overflow');
                this._htmlVisible = document.documentElement.style
                    .overflow;

                function setStyle(el, strCss) {
                    function endsWith(str, suffix) {
                        var l = str.length - suffix.length;
                        return l >= 0 && str.indexOf(suffix, l) ==
                            l;
                    }
                    var sty = el.style,
                        cssText = sty.cssText;
            ...
            if (this.slide || this.fade) {
                ...
                if (this.slide && this.main.className.indexOf(
                    ...
                    pre = '-' + pre + '-';
                    var myTransform = (('-webkit-moz-ms-o-'.indexOf(
                            pre) >= 0) ? pre : '') +
                        'transform';
                    var myTransition = (('-webkit-moz-ms-o-'.indexOf(
                            pre) >= 0) ? pre : '') +
                        'transition';

                    var xbefore = 0, ybefore = 0;
                    switch (this.slide) {
                        case 'up':
                            xbefore = this._xoffset;
                            ybefore = this._yoffset + 100;
                            break;
                        case 'down':
                            xbefore = this._xoffset;
                            ybefore = this._yoffset - 100;
                    ...
                    this.main.style.WebkitTransition = 'transform 200ms ease';
                    this.main.style.MozTransition = 'transform 200ms ease';
                    this.main.style.OTransition = 'transform 200ms ease';
                    this.main.style.transition = 'transform 200ms ease';
