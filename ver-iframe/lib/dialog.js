...
var Dialog = function(conf) {
    ...
    this.insideFrame = conf.insideFrame || false; // 是否在 iframe 里
...
Dialog.prototype = {
    init: function() {
        ...
        el.onclick = (function(event) {
            event = event ? event : (this.insideFrame ? this.constructor.FrameWin() : window).event;
    ...
    render: function(content, style, id) {
        ...
        if (!this.main || !this.wrapper) {
            ...
            this.wrapper = this.constructor.GetElementsByClassName(this.constructor
                .WrapClassName, null, el, this.doc)[0];
            this.wrapper.id = id;
            ...
            this.main = this.constructor.GetElementsByClassName(
                this.constructor.MainClassName, null, this.wrapper, this.doc)[0];
            (this.insideFrame ? this.doc.body : this.parentEl).appendChild(this.wrapper);
        }
 ...
 */
Dialog.GetElementsByClassName = function(className, tag, elm, doc) {
    if (document.getElementsByClassName) {
        _getElementsByClassName = function(className, tag, elm, doc) {
        ...
        _getElementsByClassName = function(className, tag, elm, doc) {
            ...
            var classes = className.split(" "),
                ...
                namespaceResolver = (_document.documentElement.namespaceURI === xhtmlNamespace) ? xhtmlNamespace : null,
                returnElements = [],
                elements,
                node;
            ...
            try {
                elements = _document.evaluate(".//" + tag + classesToCheck, elm, namespaceResolver, 0, null);
            } catch (e) {
                elements = _document.evaluate(".//" + tag + classesToCheck, elm, null, 0, null);
            }
...
Dialog.FrameDoc = function(parentEl, cssPath, insideFrame) {
    ...
    if (!this._FrameId) {
        ...
        if (cssPath) {
            ...
            (insideFrame ? head : (document.head || document.getElementsByTagName('head')[0])).appendChild(cssLink);
        }
...
Dialog.Visible = function(v) {
    if (!arguments.length) return this._FrameId
        //&& this.GetStyle(document.getElementById(this._FrameId), 'display') != 'none'
...
Dialog.Init = function(parentEl, cssPath, insideFrame) {
    if (document.readyState === 'complete')
        this.FrameDoc(parentEl, cssPath, insideFrame);
    ...
    })(this, parentEl, cssPath, insideFrame);
};
